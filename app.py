...
def track_transfers(df_prev, df_curr):
    # 「氏名」をキーにマージ（各ファイルで同一の氏名で登録されている前提）
...
st.markdown(
    ...
    PDFから抽出したデータを一度CSV形式に変換し、そのCSVデータを読み込んで分析を実施します。
